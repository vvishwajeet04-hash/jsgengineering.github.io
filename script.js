...
(function () {
  ...
  if (dpAddBtn) {
    dpAddBtn.addEventListener('click', async function () {
      var clientEmail = (document.getElementById('dpClientEmail') ? document.getElementById('dpClientEmail').value.trim() : '');
      ...
      var progress = document.getElementById('dpProgress') ? document.getElementById('dpProgress').value || '0' : '0';
      var notes = document.getElementById('dpNotes') ? document.getElementById('dpNotes').value.trim() : '';

      if (!client || !project || !amount) { alert('Client naam, project aur amount required hai.'); return; }

      dpAddBtn.textContent = 'Saving...';
      dpAddBtn.disabled = true;

      var clientUid = null;

      // Find UID from email via Firestore clients collection
      if (clientEmail && window.jsgDb) {
        try {
          var { collection, getDocs, query, where, addDoc } = await import('https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js');
          var q = query(collection(window.jsgDb, 'clients'), where('email', '==', clientEmail));
          var snap = await getDocs(q);
          if (!snap.empty) {
            clientUid = snap.docs[0].data().uid;
          } else {
            alert('Client email Firestore mein nahi mili. Client ne pehle Sign Up kiya hai?');
            dpAddBtn.textContent = 'Save Project';
            dpAddBtn.disabled = false;
            return;
          }

          // Save to Firestore projects collection
          await addDoc(collection(window.jsgDb, 'projects'), {
            clientUid: clientUid,
          ...
          alert('✅ Project Firestore mein save ho gaya! Client ke portal mein dikhega.');
        } catch(e) {
      ...
      // Also save to LocalStorage for owner dashboard view
      var projects = getDashProjects();
      projects.unshift({ client: client, project: project, amount: amount, paid: paid, status: status, progress: progress, notes: notes, date: new Date().toISOString() });
      saveDashProjects(projects);

      // Clear fields
      if (document.getElementById('dpClientEmail')) document.getElementById('dpClientEmail').value = '';
      ...
      if (document.getElementById('dpProgress')) document.getElementById('dpProgress').value = '';
      if (document.getElementById('dpNotes')) document.getElementById('dpNotes').value = '';
